...
//        call_user_func([$this,$data['method']],[$websocketServer,$swooleServer,$data,$frame->fd]);
     ...
     *
     * 六星教育 @shineyork老师
     * @param  WebSocketServer $swoPolarisServer [description]
     */
    protected function privateChat(WebSocketServer $swoPolarisServer, $swooleServer ,$data, $fd)
    {
        // 1. 获取私聊的id
        $clientId = $data['clientId'];
        // 2. 从redis中获取对应的服务器信息
        $clientIMServerInfoJson = $swoPolarisServer->getRedis()->hGet($this->app->make('config')->get('server.route.jwt.key'), $clientId);
        ...
        $swoPolarisServer->send($clientIMServerUrl[0], $clientIMServerUrl[1], [
            'method' => 'forwarding',
            'msg' => $data['msg'],
     ...
     * @param  WebSocketServer $swoPolarisServer [description]
     */
    protected function forwarding(WebSocketServer $swoPolarisServer, $swooleServer ,$data, $fd)
    {
